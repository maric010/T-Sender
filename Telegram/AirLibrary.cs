...
namespace Telegram
{
    ...
    public abstract class AirLibrary : ContainerControl
    {
        ...
        public BloomAir[] Colors
        {
            get
            {
                List<BloomAir> list = new List<BloomAir>();
                Dictionary<string, Color>.Enumerator enumerator = Items.GetEnumerator();
                while (enumerator.MoveNext())
                {
                    list.Add(new BloomAir(enumerator.Current.Key, enumerator.Current.Value));
                }
            ...
            set
            {
                ...
                {
                    BloomAir bloomAir = value[i];
                    if (Items.ContainsKey(bloomAir.Name))
                    {
                        Items[bloomAir.Name] = bloomAir.Value;
                    }
                }
        ...
        private void FormShown(object sender, EventArgs e)
        {
            if (!_ControlMode && !HasShown)
            {
                if (_StartPosition == FormStartPosition.CenterParent || _StartPosition == FormStartPosition.CenterScreen)
                {
                    ...
                    base.ParentForm.Location = new Point(bounds.Width / 2 - bounds2.Width / 2, bounds.Height / 2 - bounds2.Height / 2);
                }
        ...
        private void InvalidateCustimization()
        {
            ...
            BloomAir[] colors = Colors;
            foreach (BloomAir bloomAir in colors)
            {
                memoryStream.Write(BitConverter.GetBytes(bloomAir.Value.ToArgb()), 0, 4);
            }
        ...
        public void DrawRadial(ColorBlend blend, Rectangle r, int cx, int cy)
        {
            ...
            {
                CenterPoint = new PointF(r.X + cx, r.Y + cy),
                InterpolationColors = blend
            };
